    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(gate_in, SIGNAL(train_in_entrance(int,Train*)), this, SLOT(train_entering(int,Train*)));
    connect(gate_in, SIGNAL(notify_animation(int,bool,Train*)), canvas_animation, SLOT(start_animating(int,bool,Train*)));
    connect(gate_in, SIGNAL(update_cooldown_canvas(int)), this, SLOT(update_out_cooldown(int)));
    connect(train_create, SIGNAL(notify_gate_in(Train*)), gate_in, SLOT(on_new_train_notified(Train*)));
    connect(canvas_animation, SIGNAL(move_entering_on_canvas(int)), this, SLOT(move_train(int)));
    connect(canvas_animation, SIGNAL(move_exiting_on_canvas(int)), this, SLOT(move_train(int)));
    connect(canvas_animation, SIGNAL(destroy_train(int)), this, SLOT(reset_train_on_canvas(int)));
    connect(canvas_animation, SIGNAL(train_arrived_on_platform(int,Train*)), gate_in, SLOT(set_train_on_platform(int,Train*)));
    connect(this, SIGNAL(notify_gate_to_cooldown(int)), gate_in, SLOT(notified_to_remove_train(int)));
}
